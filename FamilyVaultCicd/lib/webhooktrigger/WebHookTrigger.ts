  ...
  constructor(
    ...
    authorizerLambda.addToRolePolicy(
      new PolicyStatement({
        ...
        resources: [
          "arn:aws:secretsmanager:ap-southeast-2:891377335175:secret:github-webhook-secret-869QnV",
        ],
      }),
    );
    ...
    Object.entries(pipelines).forEach(([pipelineId, pipeline]) => {
      const lambdaFunction = new NodejsFunction(
        this,
        `${pipelineId}PipelineWebhookHandlerLambda`,
        {
          runtime: lambda.Runtime.NODEJS_20_X,
          handler: "handler",
          entry: path.join(
            __dirname,
            `./lambda/${pipelineId}PipelineWebhookHandler.ts`,
          ),
          environment: {
